...
{foreach from=$danhsachquan item=foo}
    ...
    <div class="item-condition">
              <a href="danhsachnha.php?quan={$foo.districtid}" {if isset($tindang)} {if $foo.districtid eq $tindang.DistrictID}class="selectCondition"{/if}{/if} >{$foo.districtname}</a>
                 </div>
{/foreach}
...
{foreach from=$phanloainha item=foo}
  
    
    <div style="text-align:left;zoom:1;display:inline-block;*display:inline;vertical-align: top;padding-left:5px;padding-right:5px;height:20px;line-height:25px;">
               <a href="/Ha-Noi/Cho-thue-Thue-va-cho-thue-nha-c272-n242-d70.html" {if isset($tindang) && $tindang.loainhaid eq $foo.LoainhaID} class="Item selected" {else} class="Item" {/if} rel="nofollow">{$foo.TenLoaiNha}</a>
              </div>
{/foreach}
                   ...
                   <tbody>
                    <tr>
                     <td valign="top" align="left">
                      ...
                      {if isset($tindang)}
                      <div id="item_content">
                       <h1 style="padding: 5px 5px 0px 0px">{$tindang.Title }</h1>
                       <div class="admobile"><div align="center" title="Số điện thoại liên hệ của tin rao" class="center">{$tindang.PhoneNumber }</div>
                       ...
                       <div id="NewsInfo" style="margin-top:-20px">
                        <div class="NewsAuthor"><div style="float:left;width:410px;padding-left:5px">
                          <div class="detail_time">Đăng lúc: <b>{$tindang.DateTime|date_format:"%H %I"}</b>, ngày <b>{$tindang.DateTime|date_format:"%d/%m/%y"}</b> - <a href="/Ha-Noi/Thue-va-cho-thue-nha-c272.html" rel="nofollow"><b>Hà Nội</b></a></div>
                          <div class="detail_eye">Đã xem: <b>{$tindang.ViewCount}</b> . Mã Tin: <b>{$tindang.NewsID}</b> </div></div>
                         <div style="float:right;padding:0 20px 0 5px">
                          <div class="detail_personal"><a href="?action=memberinfo&memberid={$tindang.UserID}" rel="nofollow" class="a_name">{$userinfo.Name}</a><span style="margin-left:5px">|</span><span class="detail_pm" onclick="formSendPMDetail({$tindang.UserID},'{$userinfo.Name}');return false" title="Gửi tin nhắn cá nhân tới thành viên Trịnh Viết Trung">Gửi tin nhắn</span></div>
                          <div class="detail_email">
                           ...
                           -->
{/literal}
                           <a href="mailto:{$userinfo.Email}" class="a_email" rel="nofollow">{$userinfo.Email}</a>
                          </div>
                         </div>
